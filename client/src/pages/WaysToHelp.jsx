...
import "../assets/styles/WaysToHelp.css";
import { FaHandHoldingHeart, FaHandsHelping, FaHeart, FaUsers, FaGift, FaCalendarAlt, FaMapMarkerAlt, FaPhone, FaEnvelope } from "react-icons/fa";
        ...
        <div className="ways-to-help-grid">
          {waysToHelp.map((way, index) => (
            <div key={index} className="way-to-help-card" style={{ "--card-color": way.color }}>
              ...
              {way.actionTo ? (
                <Link to={way.actionTo} className="way-to-help-btn">{way.actionLabel}</Link>
              ) : (
                <Link to="/contact" className="way-to-help-btn">{way.actionLabel}</Link>
              )}
            </div>
          ))}
        </div>
